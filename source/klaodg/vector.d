...
auto To_Vec(N, O)(O v) pure nothrow if (Is_Vector!N && Is_Vector!O &&
                                        N.Dim == O.Dim) {
  N ret;
  static foreach ( i; 0 .. N.Dim )
    ret.v[i] = cast(N.Type)(v.v[i]);
  return ret;
}

auto To_Vec(N, T)(T[] v) pure nothrow if (Is_Vector!N) in {
  assert(N.Dim == v.length);
} body {
  N ret;
  static foreach ( i; 0 .. N.Dim )
    ret.v[i] = cast(N.Type)(v[i]);
  return ret;
}
